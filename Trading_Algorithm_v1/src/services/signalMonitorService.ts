...
export class SignalMonitorService {
  ...
  async triggerTestAlert(symbol: SymbolCode = 'NQ'): Promise<SignalAlert> {
    ...
    const alert: SignalAlert = {
      ...
      executionIntentId,
      source: 'MANUAL_TEST',
      title: `${candidate.symbol} ${candidate.side} manual engine test signal`,
      summary: 'Manual engine signal-path verification alert',
      candidate,
      riskDecision,
  ...
  private async evaluateSymbolAtBarFromState(
    ...
    for (const { candidate, riskDecision } of qualifyingCandidates) {
      ...
      const alert: SignalAlert = {
        ...
        title: `${candidate.symbol} ${candidate.side} manual engine signal`,
        summary: `Manual engine • ${summarizeCandidate(candidate)}`,
        candidate,
        riskDecision,
