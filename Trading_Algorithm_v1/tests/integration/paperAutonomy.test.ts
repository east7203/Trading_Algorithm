...
describe('paper autonomy integration', () => {
  it('opens paper trades directly from its own research and bar state with the personal engine disabled', async () => {
    ...
    expect(Array.isArray(paperAutonomy.recentDecisions)).toBe(true);
    expect(typeof paperAutonomy.recentDecisions[0]?.summary).toBe('string');
  });
  ...
  it('does not open autonomous futures ideas on the Good Friday holiday closure', async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'paper-autonomy-good-friday-'));
    tempDirs.push(tempDir);
    ...
    const ingest = await ctx.app.inject({
      ...
      path: '/training/ingest-bars',
      payload: {
        bars: buildAutonomyTrendBars('NQ', 420, '2026-04-03T12:30:00.000Z')
      }
    });
    ...
    expect(paperStatus.statusCode).toBe(200);
    expect(paperStatus.json().paperAccount.openTrades).toBe(0);
    expect(paperStatus.json().paperAccount.pendingEntries).toBe(0);

    const autonomyStatus = ctx.paperAutonomyService?.status();
    expect(autonomyStatus?.totalIdeas ?? 0).toBe(0);
  });

  it('does not open paper autonomy ideas from stale historical bars by default', async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'paper-autonomy-stale-default-'));
    tempDirs.push(tempDir);
